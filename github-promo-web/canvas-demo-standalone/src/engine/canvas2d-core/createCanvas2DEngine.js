...
function getMathWorkerUrl() {
  return resolveDemoAssetUrl("vendor/katex/katex-render-worker.js");
}
...
function ensureRichSelectionToolbarVariant(editingItem = null) {
  ...
  function unmount() {
    try {
      window.dispatchEvent(new CustomEvent("canvas2d-engine-unmount"));
    } catch {
      // Ignore event dispatch issues during teardown.
    }
    if (typeof cancelPendingHydrationSync === "function") {
      cancelPendingHydrationSync();
      cancelPendingHydrationSync = null;
